...
###############################################################################
##                                                                           ##
##  PURPOSE: Plot GNSS WL UPD day-boundary residual heatmap                  ##
##                                                                           ##
...
def plot_wlpdbd_out(fignam, satlst, flglst, datasec2, datasec4, mjd, mjd0, mjd1):
    """Plot wide-lane UPD day-boundary residuals by GNSS signal."""
    ...
    for k in range(flglst.shape[1]):
        data2[flglst[:, k] != 0, 6, k] = -1
        data4[flglst[:, k] != 0, :, 5, k] = -1
    ...
    for i in range(ax.shape[0]):
        for j in range(ax.shape[1]):
            ...
            sat_idx = sysidx[i][0]
            if j == 0:
                values = data2[sat_idx, 6, :]
            else:
                freq_idx = sysidx[i][j + 1][0]
                values = data4[sat_idx, freq_idx, 5, :]
            ...
            if i == 0 and j == 0:
                ...
                cbar_ax.set_xlabel(
                    "WL UPD residuals at day-boundary (cycle)",
                    fontsize=16,
                    labelpad=12,
                )
            ...
            if j == 0:
                day_values = data2[sat_idx, 6, mjd - mjd0]
            else:
                freq_idx = sysidx[i][j + 1][0]
                day_values = data4[sat_idx, freq_idx, 5, mjd - mjd0]
