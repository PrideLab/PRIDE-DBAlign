...
###############################################################################
##                                                                           ##
##  PURPOSE: Plot GNSS IRC day-boundary residual heatmap                     ##
##                                                                           ##
...
def plot_ircdbd_out(fignam, satlst, flglst, datasec3, mjd, mjd0, mjd1):
    """Plot IRC day-boundary residuals by GNSS signal."""
    doy = int(mjd_to_datetime(mjd).strftime("%j"))
    year = int(mjd_to_datetime(mjd0).strftime("%Y"))
    doy0 = int(mjd_to_datetime(mjd0).strftime("%j"))

    nday = datasec3.shape[3]
    ...
    for i in range(ax.shape[0]):
        for j in range(ax.shape[1]):
            ...
            sat_idx = sysidx[i][0]
            if j == 0:
                values = data[sat_idx, sysidx[i][j + 1][0], 7, :]
            else:
                freq_idx = sysidx[i][j + 1][0]
                values = data[sat_idx, freq_idx, 7, :]
            ...
            if i == 0 and j == 0:
                ...
                cbar_ax.set_xticks(pylab.arange(0, 0.51, 0.05))
                cbar_ax.set_xlabel(
                    "IRC residuals at day-boundary (cycle)",
                    fontsize=16,
                    labelpad=12,
                )
            ...
            if j == 0:
                day_values = data[sat_idx, sysidx[i][j + 1][0], 7, mjd - mjd0]
            else:
                freq_idx = sysidx[i][j + 1][0]
                day_values = data[sat_idx, freq_idx, 7, mjd - mjd0]
            avl_sat = day_values >= 0.0
            rms = pylab.sqrt(pylab.nanmean(day_values[avl_sat] ** 2, axis=0))
            ax[i, j].set_title(
                "Mean RMS (" + sysidx[i][j + 1][1] + ") on %d/%d: %5.2f cy"
                % (year, doy, rms),
                fontsize=16,
                pad=8,
            )
...
def main(argv):
    ...
    fignam = "fig-ircdbd.png"
    plot_ircdbd_out(
        fignam, satlst, flglst, datsec3, mjd, mjd0, mjd1
    )
