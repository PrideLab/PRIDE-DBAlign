...
###############################################################################
##                                                                           ##
##  PURPOSE: Plot GNSS orbit day-boundary discontinuity heatmap              ##
##                                                                           ##
...
def plot_orbdbd_out(fignam, satlst, flglst, datasec1, mjd, mjd0, mjd1):
    """Plot radial orbit day-boundary discontinuities by GNSS system."""
    doy = int(mjd_to_datetime(mjd).strftime("%j"))
    year = int(mjd_to_datetime(mjd0).strftime("%Y"))
    doy0 = int(mjd_to_datetime(mjd0).strftime("%j"))

    nday = datasec1.shape[2]
    prnlst = pylab.array([sat[0] for sat in satlst])
    namlst = pylab.array([sat[1] + " / " + sat[0] for sat in satlst])

    # Split satellites into GPS, Galileo, BDS-2, and BDS-3 panels.
    sysidx = [
        (select_prns(prnlst, "G"), "GPS"),
        (select_prns(prnlst, "E"), "Galileo"),
        (select_prns(prnlst, "C", max_prn=16), "BDS-2"),
        (select_prns(prnlst, "C", min_prn=17), "BDS-3"),
    ]
    ...
    norm = pylab.Normalize(vmin=0, vmax=200)

    # Normalize data and mark deleted satellites with an under-range value.
    data = abs(datasec1[:, 0, :])
    data[flglst != 0] = -1

    # Draw heatmaps.
    for i in range(len(sysidx)):
        sat_idx = sysidx[i][0]
        heatmap = ax[i].imshow(
            data[sat_idx], aspect="auto", cmap=colormap, norm=norm
        )
        ...
        if i == 0:
            ...
            cbar_ax.set_xticks(pylab.arange(0, 205, 20))
            cbar_ax.set_xlabel(
                "Radial orbit discontinuities at day-boundary (mm)",
                fontsize=16,
                labelpad=12,
            )
        ...
        if i == len(sysidx) - 1:
            ax[i].set_xlabel("DOY in %4d" % year, fontsize=16, labelpad=12)

        # Annotate daily RMS for the selected day-boundary.
        day_values = data[sat_idx, mjd - mjd0]
        avl_sat = day_values >= 0.0
        rms = pylab.sqrt(pylab.nanmean(day_values[avl_sat] ** 2, axis=0))
        ax[i].set_title(
            "Mean RMS (" + sysidx[i][1] + ") on %d/%d: %6.2f mm"
            % (year, doy, rms),
            fontsize=16,
            pad=8,
        )

        # ax[i].xaxis.grid(color='black', linestyle='dotted')
...
def main(argv):
    ...
    fignam = "fig-orbdbd.png"
    plot_orbdbd_out(
        fignam, satlst, flglst, datsec1, mjd, mjd0, mjd1
    )
